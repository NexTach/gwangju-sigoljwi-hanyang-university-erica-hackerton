...
4. 마이그레이션, 보관기간 정리, 용봉동 8개 고정 도로의 멱등 seed를 실행한다.
5. 새 Compose 릴리스를 기동하고 내부 health, 공개 health·Dashboard·용봉동
   고정 도로 응답을 확인한다.
...
두 번째 응답에는 용봉동 고정 도로 ID와 실제 도로명이 포함되어야 한다. 앱에서
산책을 시작해 자동 충격이 발생하면 익명 이동 데이터 공유가 켜진 경우 대시보드의
최근 감지와 집계가 다음 폴링 주기 안에 갱신된다. 서버 전송 실패는 앱의 산책 종료와
기기 내 리포트 저장을 막지 않는다.

기준 데이터 동기화는 별도 환경 플래그 없이 모든 배포에서 실행된다. 고정 도로를
upsert하고 도로·이동 유형별 집계가 없는 경우에만 세션과 이벤트를 보완하므로,
앱에서 이미 수집한 운영 데이터나 다른 지역 레코드를 덮어쓰거나 삭제하지 않는다.
