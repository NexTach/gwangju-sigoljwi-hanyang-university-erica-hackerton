...
```text
Flutter 앱
  간편 로그인 → 권한/약관 → 익명 UUID → 닉네임/이동 유형
  → 용봉동 내장 도로/경로 → 자동 GPS 측정 세션
  → 20~50Hz 센서 윈도우 분석 → GPS/이동 검증
  → 기기 내 리포트 저장
  → 익명 기여 ON: 세션/Barrier Candidate 실패 허용 미러링
        │
        ▼
Fastify API
  세션/이벤트 검증 → 허용된 용봉동 Road Segment 매핑
...
관리자 대시보드)을 구현한다. 모바일은 용봉동 경로·자동 GPS·센서 흐름을 포함한
하나의 기본 사용자 경험을 제공하며 별도 운영/시연 앱이나 모드로 나누지 않는다.
...
- 익명 이동 데이터 공유가 켜진 경우에만 익명 UUID, 세션 메타데이터와 후보
  이벤트를 서버에 보낸다. 서버 장애는 모바일 측정과 리포트를 막지 않는다.
- 모바일이 보내는 도로 ID 힌트는 서버의 용봉동 고정 도로 목록과 위치 근접성을
  모두 검증한 뒤 사용한다.
...
운영 seed는 용봉동 8개 고정 도로 ID를 기준으로 누락된 데이터만 보완한다.
기존 타 지역 레코드는 삭제하지 않으며, 관리자 지표와 우선순위는 용봉동 범위로
제한한다.
